...
import { createApiLogger } from '@/lib/utils/logger';  const logger = createApiLogger('StateRoute');
from 'next/server';
...
export async function GET(req: NextRequest) {
  try {
    ...
    logger.info(`Getting state for scenario ${scenarioId}...`);
    ...
    logger.info(`Successfully retrieved state for scenario ${scenarioId}`);
...
export async function POST(req: NextRequest) {
  try {
    ...
    logger.info(`Creating state for scenario ${scenarioId}...`);
    ...
    logger.info(`Successfully created state for scenario ${scenarioId}`);
